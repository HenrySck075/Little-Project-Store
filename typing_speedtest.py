...
scr.addstr(0,0,w)
lines=[i%curses.LINES for i in range(len(ws))].count(0)+3
cols=0
scr.getch()
curses.start_color()
def on_press(k):
    global idx,lines,cols
    k=k.__str__()
    k=k.replace("'",'')
    if k == 'Key.space': k=' '
    elif 'Key.' not in k:
        try:
            if k == ws[idx]: scr.addstr(k,curses.color_pair(2))
            else: scr.addstr(k,curses.color_pair(1))
            idx+=1
